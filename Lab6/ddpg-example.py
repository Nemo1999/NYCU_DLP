...
class DDPG:
    ...
    def select_action(self, state, noise_prob=0):
        ...
        if r < noise_prob:
            noise_variance = 0.1
            gaussian_noise = torch.randn((2,), device=self.device)
            action = action + noise_variance * gaussian_noise
    ...
    def _update_behavior_network(self, gamma):
        ...
        with torch.no_grad():
           ...
           q_target = reward + gamma * (1 - done.int()) * q_next
    ...
    def _update_target_network(target_net, net, tau):
        with torch.no_grad():
            ...
            for target, behavior in zip(target_net.parameters(), net.parameters()):
                ## TODO ##
                target.data = target.data * (1 - tau) + tau * behavior.data
    ...
    for episode in range(args.episode):
        ...
        for t in itertools.count(start=1):
            ...
            if total_steps < args.warmup:
                ...
                action = agent.select_action(state, noise_prob=noise_prob)
                noise_prob = max(noise_prob * args.noise_prob_decay,
                                args.noise_prob_min)
            # execute action
            ...
            if done:
                ...
                writer.add_scalar('Train/Ewma Reward', ewma_reward,
                                  total_steps)
                print(
                    'Step: {}\tEpisode: {}\tLength: {:3d}\tTotal reward: {:.2f}\tEwma reward: {:.2f}\t Noise Prob: {:.3f}'
                    .format(total_steps, episode, t, total_reward,
                            ewma_reward, noise_prob))
                break
        if episode % 100 == 0:
...
def main():
    ...
    # I add a decaying variance for action_selecting noise
    parser.add_argument('--noise_prob_decay', default=0.9999, type=float)
    parser.add_argument('--noise_prob_min', default=0.9, type=float)
    # test
